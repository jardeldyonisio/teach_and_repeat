...
# TODO: Reset the path_coords after calling the service to be able to records more than one
# path during the same node execution.

# TODO: Save path in a .csv file.
...
class TeachPathCoords(Node):
    ...
    def __init__(self):
        ...
        self.srv = self.create_service(SavePath, 'save_path', self.savePathCallback)
        ...
        threading.Thread(target=self.waitKeyPress, daemon=True).start()
    ...
    def callback(self, msg):
        self.topic_msg = msg
        if self.recording:
            self.handlingPathCoords()

    def handlingPathCoords(self):
        point = Point()
        ...
        self.markerPublisher(point)

    def markerPublisher(self, point):
        self.marker.points.append(point)
        self.marker_pub.publish(self.marker)

    def savePathCallback(self, request, response):
        try:
